...
public class SneakerDetailsActivity extends Activity {
	...
	private void setupContentByExtras() {
		...
		sneaker.setThumbnailId(getIntent().getIntExtra(SneakerEntryList.LIST_IMGID_EXTRA, 0));
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (item.getTitle() == "Edit details") {
			...
			i.putExtra(DETAILS_CATEGORY_EXTRA, sneaker.getCategory().getName().toString());
	...
	public void onActivityResult(int resultCode, int requestCode, Intent data){
		mName.setText(data.getStringExtra(
				EditSneakerEntryActivity.EDIT_NAME_EXTRA));
		mBrand.setText(data.getStringExtra(
				EditSneakerEntryActivity.EDIT_BRAND_EXTRA));
		mSellValue.setText(data.getStringExtra(
				EditSneakerEntryActivity.EDIT_SELLVAL_EXTRA));
		mRarity.setText(data.getStringExtra(
				EditSneakerEntryActivity.EDIT_RARITY_EXTRA));
		mCategory.setText(data.getStringExtra(
				EditSneakerEntryActivity.EDIT_CATEGORY_EXTRA));
		mDesc.setText(data.getStringExtra(
				EditSneakerEntryActivity.EDIT_DESC_EXTRA));
	}
}
